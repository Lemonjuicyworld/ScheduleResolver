...
// GOOD CODE (Fixed)
  ...
  try {
    ...
    final tasksJson = jsonEncode(tasks.map((t) => t.toJson()).toList());
      ...
      You are an expert student scheduling assistant. The user has provided the following tasks for their day JSON format: $tasksJson
      ...
      2. ### Ranked Tasks
      Rank which need attention first based on the urgency, importance and energy. Provide a brief reason on each.
  ...
  ScheduleAnalysis _parseResponse (String fullText) {
    ...
    for (var section in sections) {
      if (section.startsWith('Detected Conflicts'))
        ...
        (section.startsWith('Ranked Tasks'))
        rankedTasks = section.replaceFirst('Ranked Tasks','').trim();

        else if
        (section.startsWith('Recommended Schedule'))
